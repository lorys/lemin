...
/*   Updated: 2019/03/18 16:29:46 by pcarles          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
				...
				int display_rounds, t_infos *infos)
{
	...
	while (42)
	{
		...
		if (next_path == solution->paths[0] \
			&& print_newline(&status, &rounds, solution->is_start_end))
			break ;
		if (next_ant == 0)
			continue ;
	}
	if (solution->is_start_end == 1)
		fill_buffer("\n", 1);
	fill_buffer(NULL, 0);
	display_roundsf(display_rounds, rounds);
}
